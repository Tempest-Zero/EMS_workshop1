...
- **M3 SOP:** customer details; before/after media; GPS punches on all six travel legs (depart/arrive × workshop/customer + delivery) → route + fuel, with **server-side arrival verdicts** (0037: distance-from-pin + verified flag, flag-never-block) and a **tap-time soft-block** on the phone (a confident fix >250 m from the pin refuses the arrival); **breadcrumb trail** recorded per leg and drawn on the tech's map + the manager's JobDetail Leaflet map; **measured return leg** upgrades billed fuel from outbound×2; completion form (materials/time/fuel); **voice-note remark** (expo-av); **closing-video-required gate**. *Rollout note: the map tiles need the Maps API key baked into a fresh EAS build (see Pending).*
- **M4 Billing:** auto-bill on completion; **original vs negotiated** (both stored); cash ledger + corrections (void, append-only).
- **Cross-cutting:** offline covered by a generalized outbox v2 for **completion/cash/void/negotiate/punches/ready/notes** (queue offline + sync, visible failed list, survives logout). **Honest limits: closing a job is online-only by design** (the closing video must upload — an offline close would game the evidence gate; offline media queueing is a deferred phase), and media capture needs connectivity to mint the signed upload URL. Money = **integer paisa** end-to-end; every money write carries a **`client_id`** (backend dedups).

**Cut by owner (out of scope):** WhatsApp intake/bill delivery, barcode check-in, face recognition.

**Pending (operational, not build):**
- **Travel/map rollout (owner actions, in order):** (1) in Google Cloud console, create a NEW Maps key restricted to Android apps (`com.fixflow.technician` + the EAS keystore SHA-1s) and to the Maps SDK for Android only, then delete the old key that sat unrestricted in the working tree; (2) put it in `technician-app/app.json` under `android.config.googleMaps.apiKey` and commit; (3) merge the travel PRs in order (backend #137 → #138, mobile #139 → #140, web #141); (4) deploy backend from `backend/` (`railway up` — 0037 runs at boot), web via `railway up --service web`; (5) fresh **EAS build** (react-native-maps is native — `expo export` proves nothing) + device smoke: tiles render, pin-drop saves, arrival blocks >250 m with the distance shown, trail draws, return leg end-to-end, manager map shows the trail.
...
- **Offline outbox v2** (`technician-app/src/lib/outbox.ts` + `outboxSync.ts` + `useOutboxSync.ts` + `useJobOutbox.ts`): `sendOrQueue()` online-sends or queues (transient server errors — 5xx/429/timeouts — also queue); `flushOutbox()` drains on reconnect/foreground/backoff. Covers completion/payment/void/negotiate/location/**ready/notes**. Classification: definitive 4xx → a **visible failed list** on the job (Retry / Discard — never silent deletion); 401 → queue **pauses** and survives logout, resumes on re-login; items are tagged with `techId` (shared-device protection). Job Detail renders a **pending overlay** (queued payments in the ledger + balance, "syncing" badges) and warns before logging a duplicate-amount payment. Storage migrates `jobs.outbox.v1`→`v2` losslessly (corrupt v1 preserved at a backup key). Close stays online-only (evidence gate).
...
- **Migrations:** sequential Alembic 0001→**0037** (CLAUDE.md tracks the authoritative head). Recent: 0035 `job_travel_sample` (breadcrumbs) + completion fuel provenance, 0036 customer home pin + idempotent create, 0037 `job_location` verdict columns (`distance_m`/`verified`). `start.sh` **auto-runs `alembic upgrade head` on deploy**.
