...
| **J1 — Jobs core backend** *(this PR)* | `job` table (customer fields + appliance + problem + status + assigned tech + token) + seed of the 17 prototype jobs; `GET /jobs` (filter by status/tech/search), `GET /jobs/{id}`, `POST /jobs` — all auth-required | live jobs API (UI still mock) |
| **J1b — Jobs read UI** | flip manager JobsBoard + JobDetail (read) + tech MyJobs from mock → real via a web jobs data layer; action buttons deferred to J2 | manager JobsBoard + JobDetail (read), tech MyJobs |
