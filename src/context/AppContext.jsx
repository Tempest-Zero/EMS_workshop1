...
import { jobs as seedJobs, NEXT_TOKEN } from "../data/jobs";
import { technicians, techById } from "../data/technicians";
import { todayRecord } from "../data/attendance";
import { TODAY } from "../data/constants";
import { nowEntry, fmtTime } from "../lib/date";
import { estimateTotal } from "../lib/job";
...
export function AppProvider({ children }) {
  ...
  const setEstimate = useCallback(
    (jobId, { parts, laborHours, laborRate }) => {
      ...
      patchJob(
        jobId,
        (j) => ({ ...j, estimate: est }),
        nowEntry(`Estimate set: Rs ${total.toLocaleString("en-PK")} — awaiting approval`, "estimate")
      );
    },
    [patchJob]
  );
  ...
  const setEstimateStatus = useCallback(
    (jobId, status) => {
      patchJob(
        ...
        nowEntry(status === "approved" ? "Customer approved estimate" : "Customer declined estimate", status)
      );
    },
    [patchJob]
  );
  ...
  const logPayment = useCallback(
    (jobId, { amount, method }) => {
      patchJob(
        ...
        nowEntry(`Payment logged: Rs ${Number(amount).toLocaleString("en-PK")} (${method})`, "payment")
      );
  ...
  const closeJob = useCallback(
    (jobId) => {
      patchJob(jobId, (j) => ({ ...j, status: "closed", closedAt: TODAY }), nowEntry("Job closed", "status"));
  ...
  const abandonJob = useCallback(
    (jobId, reason) => {
      patchJob(
        jobId,
        (j) => ({ ...j, status: "closed", closedAt: TODAY, abandoned: true, abandonReason: reason }),
  ...
  const clockIn = useCallback((techId) => {
    ...
    setAttendanceToday((prev) => ({
      ...prev,
      [techId]: { ...prev[techId], status: prev[techId]?.status === "field" ? "field" : "present", clockedIn: true, clockIn: t, clockOut: null },
    }));
  }, []);
