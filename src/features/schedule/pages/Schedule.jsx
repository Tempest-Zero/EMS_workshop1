...
import { weekDays, assignmentsFor } from "@features/schedule/data/schedule";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <button
          className="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm font-semibold text-slate-400 hover:bg-slate-100"
          disabled
        >
      ...
      <Card className="overflow-x-auto p-0">
        <div className="min-w-[820px]">
          ...
          <div className="grid grid-cols-[140px_repeat(6,1fr)] border-b border-slate-200">
            <div className="px-3 py-2.5 text-xs font-bold uppercase tracking-wide text-slate-400">
              Technician
            </div>
            {weekDays.map((d) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`text-[11px] ${isToday ? "text-slate-300" : "text-slate-400"}`}>
                    {fmtDate(d.date)}
                  </div>
                </div>
              );
          ...
          {technicians.map((t) => (
            <div
              key={t.id}
              className="grid grid-cols-[140px_repeat(6,1fr)] border-b border-slate-100 last:border-0"
            >
              ...
              {weekDays.map((d) => {
                ...
                return (
                  <div
                    key={d.date}
                    className={`space-y-1.5 border-l border-slate-100 p-1.5 ${isToday ? "bg-slate-50" : ""}`}
                  >
                    {items.map((a) => {
                      ...
                      return (
                        <Link
                          ...
                          title={
                            visit
                              ? `${job.customer.address}${a.window ? " · " + a.window : ""}`
                              : job.customer.name
                          }
      ...
      <div className="flex items-center gap-4 text-[11px] font-medium text-slate-500">
        <span className="inline-flex items-center gap-1.5">
          <Wrench className="h-3.5 w-3.5 text-slate-400" /> Shop / bench work
        </span>
        <span className="inline-flex items-center gap-1.5">
          <Home className="h-3.5 w-3.5 text-blue-500" /> Home visit
        </span>
      </div>
    </div>
  );
