...
import { weekForTech } from "@features/schedule/data/schedule";
...
export default function MyWeek() {
  ...
  return (
    <div className="p-4 pb-8 space-y-3">
      ...
      {total === 0 && (
        <EmptyState icon={CalendarDays} title="Nothing scheduled" sub="Your week is clear." />
      )}
        ...
        return (
          <div key={d.date}>
            <div className={`mb-1.5 flex items-center gap-2 px-1 ${isToday ? "" : ""}`}>
              <span
                className={`text-sm font-extrabold ${isToday ? "text-slate-900" : "text-slate-500"}`}
              >
                {d.label}
              </span>
                ...
                {d.items.map((a) => {
                  ...
                  return (
                    <Link key={a.jobId} to={`/tech/jobs/${a.jobId}`}>
                      <Card
                        className={`p-3 active:scale-[0.99] transition ${visit ? "ring-1 ring-blue-100" : ""}`}
                      >
                        ...
                        {visit && a.window && (
                          <div className="mt-1 inline-block rounded-md bg-blue-50 px-2 py-0.5 text-[11px] font-bold text-blue-600">
                            {a.window}
                          </div>
                        )}
                      </Card>
