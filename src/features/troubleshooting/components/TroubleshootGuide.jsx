...
export default function TroubleshootGuide({ compact = false }) {
  ...
  return (
    <div className={compact ? "space-y-4 p-4 pb-8" : "space-y-5"}>
          ...
          <Card key={f.id} className="p-4">
            <div className="flex items-start justify-between gap-2">
              <div>
                <div className="flex items-center gap-2">
                  {f.code !== "—" && (
                    <span className="rounded-md bg-slate-900 px-2 py-0.5 text-xs font-extrabold text-white">
                      {f.code}
                    </span>
                  )}
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-3">
              ...
              <Button
                ...
                variant="secondary"
                onClick={() =>
                  setAddTarget({ name: f.recommendedPart, unitPrice: lowPrice(f.costRange) })
                }
              >
      ...
      <div>
        <h2 className="mb-2 mt-2 text-base font-bold tracking-tight text-slate-800">
          Common Fixes
        </h2>
      ...
      <div className="space-y-4">
        ...
        <Field label="Unit Price (Rs)">
          <input
            ...
            placeholder={String(target.unitPrice)}
          />
        </Field>
