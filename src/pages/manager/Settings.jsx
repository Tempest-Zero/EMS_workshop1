import { Building2, Bell, CreditCard, Users2, Link2, ShieldCheck } from "lucide-react";
import { Card, SectionHeader, Field, inputClass, Button } from "../../components/primitives";
import IntegrationBadge from "../../components/IntegrationBadge";
import { WORKSHOP } from "../../data/constants";

const INTEGRATIONS = [
  { name: "Attendance Service", desc: "Biometric clock-in sync", icon: Users2, status: "Connected" },
  { name: "Payroll Service", desc: "Monthly salary & deductions", icon: CreditCard, status: "Connected" },
  { name: "SMS Gateway", desc: "Customer ready / pickup alerts", icon: Bell, status: "Connected" },
  { name: "Parts Supplier Catalog", desc: "Live part pricing & orders", icon: Link2, status: "Connected" },
];
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Working Days / Month">
            <input className={inputClass} type="number" defaultValue={WORKSHOP.workingDaysThisMonth} />
          </Field>
        </div>
        <div className="mt-4">
      ...
      <Card className="p-5">
        ...
        <div className="mt-3 space-y-2">
          {INTEGRATIONS.map((it) => {
            const Icon = it.icon;
            return (
              <div key={it.name} className="flex items-center gap-3 rounded-xl border border-dashed border-slate-300 bg-slate-50/70 px-4 py-3">
