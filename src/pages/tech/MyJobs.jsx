...
import { Home, ChevronRight, Clock3, Phone } from "lucide-react";
import { useApp } from "../../context/AppContext";
import { Card, EmptyState } from "../../components/primitives";
import StatusChip from "../../components/StatusChip";
import { formatPKR } from "../../lib/currency";
import { amountOwed, hasEstimate } from "../../lib/job";
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`flex-1 rounded-lg px-3 py-2 text-sm font-bold transition ${
              filter === f.key ? "bg-slate-900 text-white" : "bg-white text-slate-500 ring-1 ring-inset ring-slate-200"
            }`}
          >
            {f.label}
          </button>
                ...
                <Card className="p-4 active:scale-[0.99] transition">
                  ...
                  <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3">
                    <span className="text-sm font-bold text-slate-700">
                      {hasEstimate(j) ? formatPKR(amountOwed(j)) : <span className="text-slate-400">No estimate</span>}
                    </span>
