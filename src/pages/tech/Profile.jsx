  ...
  return (
    <div className="p-4 pb-8 space-y-4">
      ...
      <Card className="p-5 text-center">
        ...
        <a href={`tel:${t.phone}`} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 py-3 text-sm font-bold text-white">
      ...
      {/* Pay summary */}
      <Card className="p-4">
        <SectionHeader title="Pay Summary" sub={latest.month} action={<Wallet className="h-5 w-5 text-slate-300" />} />
        <div className="mt-3 rounded-xl bg-slate-900 p-4 text-white">
          <div className="text-xs font-semibold uppercase tracking-wide text-slate-400">Net Pay</div>
          <div className="text-3xl font-extrabold">{formatPKR(latest.net)}</div>
        </div>
        <dl className="mt-3 space-y-1.5 text-sm">
          <div className="flex justify-between"><dt className="text-slate-500">Base salary</dt><dd className="font-semibold text-slate-700">{formatPKR(latest.base)}</dd></div>
          <div className="flex justify-between"><dt className="text-slate-500">Days worked</dt><dd className="font-semibold text-slate-700">{latest.daysWorked}</dd></div>
          <div className="flex justify-between"><dt className="text-slate-500">Deductions</dt><dd className="font-semibold text-red-500">{latest.deductions ? `−${formatPKR(latest.deductions)}` : "—"}</dd></div>
          <div className="flex justify-between"><dt className="text-slate-500">Advances taken</dt><dd className="font-semibold text-amber-600">{latest.advances ? `−${formatPKR(latest.advances)}` : "—"}</dd></div>
        </dl>
