  ...
  return (
    <div className="p-4 pb-8 space-y-4">
      ...
      <Card className={`p-6 text-center ${isIn ? "bg-emerald-50 ring-1 ring-emerald-200" : ""}`}>
        <div className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full ${isIn ? "bg-emerald-100 text-emerald-600" : "bg-slate-100 text-slate-400"}`}>
        ...
        {isIn ? (
          <>
            ...
            <div className="mt-0.5 text-xs font-semibold text-slate-400">{elapsedSince(rec.clockIn)} elapsed</div>
            <button
              onClick={() => { clockOut(currentTechId); addToast("Clocked out", "default"); }}
          ...
          <>
            ...
            <button
              onClick={() => { clockIn(currentTechId); addToast("Clocked in — have a good shift", "ready"); }}
      ...
      {/* Month calendar */}
      <Card className="p-4">
        <SectionHeader title="This Month" sub={`${summary.present} of ${summary.working} days present`} />
        <div className="mt-3">
