...
| **Mobile** (`technician-app/`)                     | Expo **SDK 52**, React Native 0.76.9, **Android-only**, TypeScript strict, Jest (`jest-expo`); needs an **EAS dev build** (not Expo Go). **Background geofencing** (`expo-task-manager` + `ACCESS_BACKGROUND_LOCATION`) drives the attendance arrival/exit prompts. Crash reporting = `@sentry/react-native`, gated on `EXPO_PUBLIC_SENTRY_DSN` (unset = no-op via the `?? ""` gate in `sentry.ts`); to enable, add the key **with a real DSN value** to the `eas.json` env blocks — **never as an empty string** (`eas.json` rejects an empty env value and every `eas build` fails, which CI's `expo export` won't catch). **Never add the Sentry Expo config plugin to `app.json`** — it breaks the gradle build, also uncaught by `expo export`                                                                           | not iOS, not Expo Go                                                                                      |
