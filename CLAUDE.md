...
(`ARCHITECTURE.md`, `docs/*`) explain the *why* but drift over time. This file is the
...
| **Backend** (`backend/`) | **FastAPI** (Python 3.12), SQLAlchemy 2.0 **async** + asyncpg, **Alembic** migrations (head = **0017**), Pytest + pytest-asyncio, Ruff, Mypy strict, import-linter | not Django, not sync SQLAlchemy |
...
| **Auth** | **Custom FastAPI JWT** — name/PIN login, PBKDF2-hashed PIN, `token_version` revocation, per-IP throttle + lockout (`backend/app/features/identity/`). Authorization enforced in the **service layer** (`CurrentPrincipal`/`CurrentManager`) | **NOT** Supabase GoTrue |
...
| **Push** | Firebase FCM | — |
| **Deploy** | **Railway**, **manual `railway up`** (NOT GitHub-connected). Mobile via **EAS Build** | not auto-deploy on merge |
...
- **Effectively single-tenant today**: `shop_id` columns exist but the code uses a
  `DEFAULT_SHOP_ID` constant. Multi-customer is a *future* goal — don't build for it as if it's live.
- **`expo export` (in CI) bundles JS only** — it does NOT catch native/Gradle compile breaks
  (e.g. a misconfigured Sentry gradle plugin). A real Android compile is currently *not* gated.
...
| `docs/SOLUTION-ARCHITECT-GUIDE.md` | The *why* behind decisions; where a new capability belongs |
