...
async def test_media_lifecycle(app_client: AsyncClient) -> None:
    job = "job-int-1"
    ...
    listed = await app_client.get(f"/api/jobs/{job}/media")
    ...
    completed = await app_client.post(
        f"/api/jobs/{job}/media/{media_id}/complete", json={"size_bytes": 1000}
    )
    ...
    deleted = await app_client.delete(f"/api/jobs/{job}/media/{media_id}")
    assert deleted.status_code == 204, deleted.text
    listed_after = await app_client.get(f"/api/jobs/{job}/media")
