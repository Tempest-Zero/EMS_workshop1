"""Ops slice business logic — the two read-only surfaces only the backend can
produce.

Health and metrics are computed from local machinery (DB session, R2 storage,
the in-process metrics registry, the APScheduler handle). Railway logs/deploys/
metrics and the Sentry issues feed are owned by the standalone ops server, not
this backend — see ``ops-server.mjs``.
"""
